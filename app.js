...
loginBtn.onclick=async()=>{if(!db)return;M.textContent="";let {error}=await db.auth.signInWithPassword({email:document.getElementById("email").value.trim(),password:document.getElementById("password").value});if(error)M.textContent="Confira e-mail e senha."}
signupBtn.onclick=async()=>{if(!db)return;let {error}=await db.auth.signUp({email:document.getElementById("email").value.trim(),password:document.getElementById("password").value});M.textContent=error ? "Erro: " + error.message : "Conta criada. Confira seu e-mail.";}
